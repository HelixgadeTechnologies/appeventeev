...
const ExistingTicket = ({ handleEdit, type }) => {
  ...
  if (!tickets.length) return <p className="text-center py-10">No {type} tickets found</p>;
  ...
  return (
    <div className={`w-full py-5 ${tickets.length === 1 ? 'mx-auto' : ''}`}>
          ...
          return (
            <div
              ...
              className={`rounded-md p-1 ${
                tickets.length === 1 ? 'w-full' : tickets.length === 2 ? 'w-[47%]' : 'w-[32%]'
              }`}
            >
              <div
                className={`rounded-md ${tickets.length === 1 ? 'py-10 oneTicketGrid px-3' : 'py-5 px-3 space-y-3'}`}
              ...
              >
                ...
                <div className={`flex justify-between items-center mt-2 smallFont ${tickets.length === 1 ? 'px-5' : ''}`}>
                  <p className="text-sm text-orange-600">{ticket.quantity} Available</p>
                ...
                <div className="flex justify-between items-center mt-2 smallFont font-semibold">
                  <p className="text-orange-600">{sold} Sold</p>
                  <p>Revenue: ${sold * ticket.price}</p>
                  {tickets.length === 1 && (
                    <button
                      onClick={() => handleEdit(ticket._id)}
                      className="bg-orange-500 text-white px-2 py-1 rounded-sm text-xs"
